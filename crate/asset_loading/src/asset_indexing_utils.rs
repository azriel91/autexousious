...
impl AssetIndexingUtils {
    ...
    pub fn into_asset_record(namespace: String, path: PathBuf) -> Option<AssetRecord> {
...
mod tests {
    ...
    fn returns_none_when_namespace_invalid() {
        assert_eq!(
            None,
            AssetIndexingUtils::into_asset_record(
                " invalid".to_string(),
    ...
    fn returns_none_when_path_invalid() {
        assert_eq!(
            None,
            AssetIndexingUtils::into_asset_record("user1".to_string(), PathBuf::from("/"))
        );
    }
