...
/// Utility functions for checking if game input matches the direction the
/// character is facing.
///
/// New consumers should prefer functions on `game_input_model::InputDirection`
/// instead.
#[derive(Debug)]
pub struct SequenceHandlerUtil;

impl SequenceHandlerUtil {
    /// Returns whether X axis input is in the same direction as the object is
    /// facing.
    ///
    ...
    /// Returns whether X axis input is in the opposite direction as the object
    /// is facing.
    ///
    /// This returns `false` if there is no input on the X axis.
    ///
