...
pub enum SessionMessageEvent {
    ...
    ///
    /// The session server waits for this message from all game clients before sending all `GameInputEvent`s to clients.
    ///
    /// Clients waits for this message from the game server before ticking the game.
    GameInputTick,
